...
const Cart = ({ coupons, productData, infoData, categories }) => {
  ...
  const handleValidCoupon = () => {
    ...
    const matchingCoupon = coupons.find(
      (couponObj) => couponObj.coupon.toLowerCase() === enteredCoupon
    );
    ...
    if (!matchingCoupon) {
      ...
      if (finalPrice >= couponTotal) {
        ...
        const discount =
          (parseFloat(matchingCoupon.percentageDiscount) / 100) * finalPrice;
                    ...
                    {/* <div className="flex justify-between border-t">
                      <div className="mt-3 text-md md:text-lg font-italic text-black-300">
                        *Get free delivery on order above{" "}
                        <b>&#8377;{infoData[0].freeDelivery}</b>
                      </div>
                    </div>
                    <div className="flex justify-between border-t">
                      <div className="mt-3 text-md md:text-lg font-italic text-black-300">
                        *Delivery Charges per order:{" "}
                        <b>&#8377;{infoData[0].deliveryCharges}</b>
                      </div>
                    </div> */}
                  </div>
